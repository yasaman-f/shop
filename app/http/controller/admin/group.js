...
class GroupController extends Controller {
  async addGroup (req, res, next) {
    try {
      ...
      if(await this.findGroup(title)) throw Error.BadRequest("Grouping already exists") 
  ...
  async editGroup (req, res, next) {
    try {
      ...
      console.log(update);
      if(!update.modifiedCount) throw Error.InternalServerError("The grouping was not edited")
      return res.status(HttpStatus.OK).json({
        StatusCode: HttpStatus.OK,
        data: {
